...
const createAddress = async (user_id, data) => {
  try {
    ...
    if (!full_name || !address || !phone) {
      return { status: "ERROR", message: "Thiếu thông tin bắt buộc (full_name, address, phone)." };
    }
...
const updateAddress = async (user_id, address_id, data) => {
  try {
    if (!user_id) {
      return { status: "ERROR", message: "Bạn cần đăng nhập để cập nhật địa chỉ." };
    }
    // Kiểm tra địa chỉ có tồn tại không
    const address = await UserAddress.findOne({ _id: address_id, user_id });

    if (!address) {
      return { status: "ERROR", message: "Không tìm thấy địa chỉ hoặc bạn không có quyền chỉnh sửa." };
    }
    // Không cho phép cập nhật user_id
    if (data.user_id) {
      return { status: "ERROR", message: "Không thể thay đổi quyền sở hữu địa chỉ." };
    }
    const updatedAddress = await UserAddress.findByIdAndUpdate(address_id, data, { new: true });
...
const getDetailAddress = async (user_id, address_id) => {
  try {
    ...
    const address = await UserAddress.findOne({ _id: address_id, user_id, is_deleted: false });

    if (!address) {
      return { status: "ERROR", message: "Không tìm thấy địa chỉ hoặc bạn không có quyền truy cập." };
    }

    return { status: "OK", message: "Lấy chi tiết địa chỉ thành công.", data: address };
  } catch (error) {
...
const deleteAddress = async (user_id, address_id) => {
  try {
    ...
    if (!address) {
      return { status: "ERROR", message: "Không tìm thấy địa chỉ hoặc bạn không có quyền xóa." };
    }
