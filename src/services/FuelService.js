...
const getAllFuel = async (filters) => {
  try {
    ...
    // Kết quả
    const requests = await FuelTypes.find(query).sort({ createdAt: -1 });
  ...
  const updateFuel = async (id, data) => {
    try {
      const updatedFuel = await FuelTypes.findByIdAndUpdate(
        id,
        { 
  ...
  const cancelFuel = async (id) => {
    try {
      const canceledFuel = await FuelTypes.findByIdAndUpdate(
        id,
