...
const createFuelStorageReceipt = async (
  ...
  try {
    ...
    if (receipt_request_id) {
        await MaterialCollectionRequest.findByIdAndUpdate(receipt_request_id, { status: "Đang xử lý" });
    } else if (receipt_supply_id) {
        await MaterialProvideRequest.findByIdAndUpdate(receipt_supply_id, { status: "Đang xử lý" });
    }
...
const getQuantityByReceiptId = async (
  ...
  try {
    if (receipt_supply_id) {
      const supplyOrder = await MaterialProvideRequest.findById(receipt_supply_id);
      if (!supplyOrder) throw new Error("Không tìm thấy đơn hàng cung cấp!");
      return supplyOrder.quantity;
    }

    if (receipt_request_id) {
      const requestOrder = await MaterialCollectionRequest.findById(receipt_request_id);
